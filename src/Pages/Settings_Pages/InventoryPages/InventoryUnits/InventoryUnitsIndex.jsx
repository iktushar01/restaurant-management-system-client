...
const InventoryUnitsIndex = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [entriesToShow, setEntriesToShow] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);

  const { data: units, totalEntries, totalPages, loading, error, refetch, startIndex } = useApiList(
  ...
  const handleDeleteUnit = async (id) => {
    ...
    if (!ok) return;
    try {
        await inventoryService.units.delete(id);
        refetch();
      } catch (err) {
  ...
  return (
    <div className="p-6   mx-auto">
      ...
      {loading ? <div className="text-center py-12 text-muted-foreground">Loading...</div> : (
        <ReusableTable columns={columns} data={units} actions={actions} />
      )}
      ...
      {units.length === 0 && (
        <div className="bg-card rounded-xl shadow-sm border border-border p-6 sm:p-8 md:p-12 text-center mt-8">
          ...
          <p className="text-muted-foreground mb-6">Get started by adding a new unit</p>
          <Link to="create">
...
export default InventoryUnitsIndex;

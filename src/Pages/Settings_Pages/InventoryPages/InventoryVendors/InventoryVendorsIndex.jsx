...
const InventoryVendorsIndex = () => {
  ...
  const handleDeleteSupplier = async (id) => {
    const ok = await confirm({ description: "Are you sure you want to delete this supplier?" });
    if (!ok) return;
    try {
  ...
  const actions = [
    ...
    },
    {
      ...
      onClick: (row) => handleDeleteSupplier(row.id),
      render: (row) => (
        <button
...
export default InventoryVendorsIndex;

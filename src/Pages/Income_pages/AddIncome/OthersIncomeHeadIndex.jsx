...
const OthersIncomeHeadIndex = () => {
  ...
  const handleDelete = async (id) => {
    const ok = await confirm({ description: "Are you sure you want to delete this income head?" });
    if (!ok) return;
    try {
  ...
  const columns = [
    ...
    { header: "Income Head Name", accessor: "incomeheadname" },
    {
      ...
      render: (row) =>
        row.description ? row.description : <span className="text-muted-foreground">Not specified</span>,
    },
  ];
    ...
    <div className="p-6   mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8 bg-muted/40 sm:bg-muted/40 p-4 sm:p-6 rounded-xl">
        <div>
          ...
          <p className="text-muted-foreground text-sm sm:text-base mt-1">Manage your organization's Others Income</p>
        </div>

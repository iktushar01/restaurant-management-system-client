...
const InventoryStockLocationCreate = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setSubmitting(true);
    try {
      await inventoryService.stockLocations.create({ name: data.name });
      navigate("/WorkPeriod/inventory/stock-locations");
    } catch (err) {
  ...
  return (
    <div className="   mx-auto p-6">
      <div className="flex items-center mb-6">
        <Link to="/WorkPeriod/inventory/stock-locations" className="flex items-center group transition-all duration-200">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
        <div className="p-6 bg-gradient-to-r bg-primary text-primary-foreground border-b border-border">
          <h2 className="text-2xl font-bold text-foreground">Add Stock Location</h2>
          <p className="text-foreground mt-1">Fill in the details below to add a new stock location</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            <div>
              ...
              <input type="text" className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus-visible:ring-ring focus-visible:border-ring outline-none" placeholder="Enter location name"
                {...register("name", { required: "Location name is required", minLength: { value: 2, message: "Location name must be at least 2 characters" } })} />
              {errors.name && <p className="mt-1 text-sm text-destructive">{errors.name.message}</p>}
            </div>
          </div>

          <div className="mt-8 flex justify-end space-x-3">
            <Link to="/WorkPeriod/inventory/stock-locations" className="px-5 py-2.5 border border-border rounded-lg text-foreground hover:bg-muted/40">Close</Link>

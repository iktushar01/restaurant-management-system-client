...
const BranchInfoIndex = () => {
  ...
  const handleDelete = async (id) => {
    const ok = await confirm({ description: "Are you sure you want to delete this branch?" });
    if (!ok) return;
    try {
  ...
  const columns = [
    ...
    { header: "Branch Name", accessor: "branchName" },
    {
      ...
      accessor: "address",
      render: (row) =>
        row.address ? row.address : <span className="text-muted-foreground">Not specified</span>,
    },
  ];
  ...
  return (
    <div className="p-6   mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8 bg-muted/40 sm:bg-muted/40 p-4 sm:p-6 rounded-xl">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Branch Info</h1>
          <p className="text-muted-foreground text-sm sm:text-base mt-1">Manage your organization's Branch info</p>
        </div>

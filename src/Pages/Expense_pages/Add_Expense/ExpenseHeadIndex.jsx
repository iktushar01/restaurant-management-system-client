...
const ExpenseHeadIndex = () => {
  ...
  const { data: heads, totalEntries, totalPages, loading, error, refetch, startIndex } = useApiList(
    financeService.expenseHeads.getAll,
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this expense head?")) {
      try {
        await financeService.expenseHeads.delete(id);
        refetch();
      } catch (err) {
  ...
  const columns = [
    ...
    { header: "Expense Head Name", accessor: "expenseheadname" },
    {
      ...
      render: (row) => row.description ? row.description : <span className="text-gray-400">Not specified</span>,
    },
  ];
  ...
  const actions = [
    {
      ...
      render: (row) => (
        <Link to={`/expense/ExpenseHead/Edit/${row.id}`} className="flex items-center space-x-1">
          <FaEdit />
          <span>Edit</span>
        </Link>
      ),
  ...
  return (
    <div className="p-6 max-w-6xl min-h-screen mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8 bg-gray-50 sm:bg-gray-100 p-4 sm:p-6 rounded-xl">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Expense Head</h1>
          <p className="text-gray-500 text-sm sm:text-base mt-1">Manage your organization's expense types</p>
        </div>

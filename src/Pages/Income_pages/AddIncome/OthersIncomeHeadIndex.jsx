...
const OthersIncomeHeadIndex = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const [entriesToShow, setEntriesToShow] = useState(10);
  const [currentPage, setCurrentPage] = useState(1);

  const { data: heads, totalEntries, totalPages, loading, error, refetch, startIndex } = useApiList(
    financeService.incomeHeads.getAll,
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this income head?")) {
      try {
        await financeService.incomeHeads.delete(id);
        refetch();
      } catch (err) {
  ...
  const columns = [
    ...
    { header: "Income Head Name", accessor: "incomeheadname" },
    {
      header: "Description",
      accessor: "description",
      render: (row) =>
        row.description ? row.description : <span className="text-gray-400">Not specified</span>,
    },
  ];
  ...
  const actions = [
    {
      ...
      render: (row) => (
        <Link to={`/income/OthersIncomeHead/Index/Edit/${row.id}`} className="flex items-center space-x-1">
          <FaEdit />
          <span>Edit</span>
        </Link>
      ),
    },
    {
      ...
      render: (row) => (
        <button onClick={() => handleDelete(row.id)} className="flex items-center space-x-1 text-rose-600 hover:text-rose-900">
          <FaTrash />
          <span>Delete</span>
        </button>
      ),
  ...
  return (
    <div className="p-6 max-w-6xl min-h-screen mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8 bg-gray-50 sm:bg-gray-100 p-4 sm:p-6 rounded-xl">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Others Income</h1>
          <p className="text-gray-500 text-sm sm:text-base mt-1">Manage your organization's Others Income</p>
        </div>
      ...
      {loading ? <div className="text-center py-12 text-gray-500">Loading...</div> : (
        <ReusableTable columns={columns} data={heads} actions={actions} />
      )}
...
export default OthersIncomeHeadIndex;

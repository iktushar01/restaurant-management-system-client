...
const BankInfoIndex = () => {
  ...
  const { data: banks, totalEntries, totalPages, loading, error, refetch, startIndex } = useApiList(
    bankService.banks.getAll,
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this bank?")) {
      try {
        await bankService.banks.delete(id);
        refetch();
      } catch (err) {
  ...
  const actions = [
    {
      ...
      render: (row) => (
        <Link to={`/bank/bankinfo/Index/Edit/${row.id}`} className="flex items-center space-x-1">
          <FaEdit />
          <span>Edit</span>
        </Link>
      ),
  ...
  return (
    <div className="p-6 max-w-6xl min-h-screen mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8 bg-gray-50 sm:bg-gray-100 p-4 sm:p-6 rounded-xl">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Bank Info</h1>
          <p className="text-gray-500 text-sm sm:text-base mt-1">Manage your organization's bank info</p>
        </div>
      ...
      {loading ? <div className="text-center py-12 text-gray-500">Loading...</div> : (
        <ReusableTable columns={columns} data={banks} actions={actions} />
      )}

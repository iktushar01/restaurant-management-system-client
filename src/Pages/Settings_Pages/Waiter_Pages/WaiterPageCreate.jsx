...
const WaiterPageCreate = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setSubmitting(true);
    try {
      await waiterService.create({ name: data.name, note: data.note || "" });
      navigate("/WorkPeriod/RestaurantDineWaiter/Index");
    } catch (err) {
      setSubmitError(err.message || "Failed to create waiter");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="   mx-auto p-6">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
        <div className="p-6 bg-gradient-to-r bg-primary text-primary-foreground border-b border-border">
          <h2 className="text-2xl font-bold text-foreground">Add New Waiter</h2>
          <p className="text-foreground mt-1">Fill in the details below to add a new waiter</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Waiter Name</label>
              <input type="text" className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus-visible:ring-ring focus-visible:border-ring outline-none" placeholder="Enter waiter name"
                {...register("name", { required: "Waiter name is required", minLength: { value: 2, message: "Name must be at least 2 characters" } })} />
              {errors.name && <p className="mt-1 text-sm text-destructive">{errors.name.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Note</label>
              <textarea className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus-visible:ring-ring focus-visible:border-ring outline-none" placeholder="Add any notes" rows={3} {...register("note")} />
            </div>
          </div>

          <div className="mt-8 flex justify-end space-x-3">
            <Link to="/WorkPeriod/RestaurantDineWaiter/Index" className="px-5 py-2.5 border border-border rounded-lg text-foreground hover:bg-muted/40">Close</Link>

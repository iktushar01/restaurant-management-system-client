...
const BankInfoIndex = () => {
  ...
  const handleDelete = async (id) => {
    const ok = await confirm({ description: "Are you sure you want to delete this bank?" });
    if (!ok) return;
    try {
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 sm:mb-8 bg-muted/40 sm:bg-muted/40 p-4 sm:p-6 rounded-xl">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-foreground">Bank Info</h1>
          <p className="text-muted-foreground text-sm sm:text-base mt-1">Manage your organization's bank info</p>
        </div>

...
const ExpenseHeadCreate = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setSubmitting(true);
    try {
      await financeService.expenseHeads.create({
        name: data.expensehead,
        description: data.description || "",
      });
      navigate("/expense/ExpenseHead/Index");
    } catch (err) {
      setSubmitError(err.message || "Failed to create expense head");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="  mx-auto p-6">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
        <div className="p-6 bg-gradient-to-r bg-primary text-primary-foreground border-b border-border">
          <h2 className="text-2xl font-bold text-foreground">Add Expense Heading</h2>
          <p className="text-foreground mt-1">Fill in the details below to create a new Expense Heading</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          <div className="mt-8 flex justify-end space-x-3">
            <Link to="/expense/ExpenseHead/Index" className="px-5 py-2.5 border border-border rounded-lg text-foreground hover:bg-muted/40">Cancel</Link>

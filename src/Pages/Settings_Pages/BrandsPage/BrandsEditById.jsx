...
const BrandsEditById = () => {
  ...
  useEffect(() => {
    inventoryService.brands.getById(id).then((res) => {
      setValue("name", res.data.name);
      setValue("details", res.data.details || "");
    }).catch((err) => setSubmitError(err.message || "Failed to load brand"))
  ...
  const onSubmit = async (data) => {
    ...
    setSubmitting(true);
    try {
      await inventoryService.brands.update(id, { name: data.name, details: data.details || "" });
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div>
              ...
              <textarea className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-300 focus:border-amber-300 outline-none" rows={3} {...register("details")} />
            </div>
          </div>

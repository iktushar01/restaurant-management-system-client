...
const InventoryVendorsCreate = () => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({ defaultValues: { openingBalance: 0 } });

  const onSubmit = async (data) => {
    setSubmitError("");
    setSubmitting(true);
    try {
      await inventoryService.vendors.create({
        name: data.name,
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden">
        <div className="p-6 bg-gradient-to-r bg-primary text-primary-foreground border-b border-border">
          <h2 className="text-2xl font-bold text-foreground">New Supplier</h2>
          <p className="text-foreground mt-1">Fill in the details below to add a new supplier</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          <div className="grid grid-cols-1 gap-6">
            <div>
              ...
              <input type="text" className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus-visible:ring-ring focus-visible:border-ring outline-none" placeholder="Enter supplier name"
                {...register("name", { required: "Supplier name is required", minLength: { value: 2, message: "Supplier name must be at least 2 characters" } })} />
              {errors.name && <p className="mt-1 text-sm text-destructive">{errors.name.message}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Address</label>
              <input type="text" className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus-visible:ring-ring focus-visible:border-ring outline-none" placeholder="Enter supplier address" {...register("address")} />
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Contact No</label>
              <input type="text" className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus-visible:ring-ring focus-visible:border-ring outline-none" placeholder="Enter contact number" {...register("contact")} />
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Opening Balance</label>
              <input type="number" step="0.01" className="w-full px-4 py-2.5 border border-border rounded-lg focus:ring-2 focus-visible:ring-ring focus-visible:border-ring outline-none" placeholder="Enter opening balance"
                {...register("openingBalance", { min: { value: 0, message: "Opening balance cannot be negative" } })} />

...
const ExpenseEditById = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      financeService.expenseEntries.getById(id),
    ])
      ...
      .catch((err) => setSubmitError(err.message || "Failed to load expense"))
  ...
  const onSubmit = async (data) => {
    ...
    setSubmitting(true);
    try {
      await financeService.expenseEntries.update(id, {
        headId: data.headId,
      ...
      setSubmitError(err.message || "Failed to update expense");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="max-w-6xl min-h-screen mx-auto p-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-6 bg-gradient-to-r from-yellow-200 to-yellow-400 border-b border-gray-200">
          ...
          <p className="text-gray-700 mt-1">Update the expense details below</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          ...
          <div className="mt-8 flex justify-end space-x-3">
            <Link to="/expense/manage" className="px-5 py-2.5 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Close</Link>
